...
import type { ArticleLimitedType, ArticleType, ChapterType } from '$lib/types.js';
		...
		return await formRunner(
			...
			async (event, formData, cookies, user) => {
				//try {
					...
					const chapters: ChapterType[] = JSON.parse(formData['chapters']);
					const articles: ArticleLimitedType[][] = JSON.parse(formData['articles']);
