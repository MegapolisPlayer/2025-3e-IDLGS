...
>
	<div class="flex h-[4svh] {isInTextbookOrCourse ? 'min-w-3/10 grow' : 'min-w-1/8'} flex-row">
		...
		<div
			...
			style="background-image: radial-gradient(circle at 0% 100%, transparent 0%, transparent 5svh, var(--color-blue-900) 5svh, var(--color-blue-900) 100%);"
		></div>
	</div>

	<div
		class="
			flex h-[4svh] w-6/8 flex-row items-center gap-0 *:flex-nowrap *:text-nowrap max-xl:hidden *:xl:gap-2 *:2xl:gap-6
			 {isInTextbookOrCourse ? 'bg-white' : ''} rounded-b-xl
			"
	>
		<div
			class="
			flex h-[4svh] flex-row items-center gap-1 rounded-b-xl bg-blue-900 ps-4 pe-4
		"
		>
		...
		<div
			class="flex h-[4svh] flex-row items-center gap-1 rounded-b-xl bg-cyan-600 ps-4 pe-4"
		>
			<NavbarLink
				link="/settings"
				emoji="user-3"
	...
	<div class="flex h-[4svh] min-w-1/8 flex-row">
		<div
			class="aspect-square h-full w-auto"
			style="background-image: radial-gradient(circle at 100% 100%, transparent 0%, transparent 5svh, var(--color-cyan-600) 5svh, var(--color-cyan-600) 100%);"
		></div>
		<div class="grow"></div>
	</div>
</nav>
