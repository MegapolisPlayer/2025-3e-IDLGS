...
export const load = async (event) => {
	const success = await event.locals.db.transaction(async (tx) => {
		await tx
			...
			.where(lte(schema.passwordReset.expiresAt, new Date()));

		const data = await tx
			.select()
			.from(schema.passwordReset)
			.where(eq(schema.passwordReset.uuid, event.params.changeUuid))
			.limit(1);
	...
	setPassword: async () => {
		return await formRunner(
			...
			async (event, formData) => {
				...
				try {
					const success = await event.locals.db.transaction(
						async (tx) => {
							const user = await tx
								.delete(schema.passwordReset)
								.where(
									eq(
										schema.passwordReset.uuid,
										formData['uuid'],
									),
								)
								.returning();
					...
					writeLog(
						event,
						'ERROR',
						'DB failure during password change.',
					);
					return fail(500);
				}
			},
